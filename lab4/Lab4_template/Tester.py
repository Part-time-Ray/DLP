...
class Test_model(VAE_Model):
    ...
    def val_one_step(self, img, label, idx=0):
        ...
        for i in range(1, 630):
            pre_img = decoded_frame_list[-1].to(self.args.device)
            label_frame = label[i].to(self.args.device)
            gen_img = self(pre_img, label_frame)
            # print("decoded_frame_list[-1].shape", decoded_frame_list[-1].shape)
            decoded_frame_list.append(gen_img.cpu())
        ...
        assert label_frame.shape == (1, 630, 3, 32, 64), f"The shape of output should be (1, 630, 3, 32, 64), but your output shape is {label_frame.shape}"
    ...
    def load_checkpoint(self):
        if self.args.ckpt_path != None:
            print("Loading checkpoint from {}".format(self.args.ckpt_path))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_root',     type=str, default='./save_path',  help="The path to save your data")
