...
class MaskGIT:
    def __init__(self, args, MaskGit_CONFIGS):
        ...
        checkpoint = torch.load(args.load_transformer_ckpt_path, weights_only=False)
        self.model.transformer.load_state_dict(checkpoint['transformer_state_dict'])
        self.model.eval()
        self.num_image_tokens = MaskGit_CONFIGS["model_param"]["num_image_tokens"]
...
#TODO3 step1-2: modify the path, MVTM parameters
    ...
    i=0
    tq = tqdm.tqdm(zip(t.mi_ori, t.mask_ori))
    for image, mask in tq:
        ...
        i+=1
        tq.set_description(f"Processing image {i}/{len(t.mi_ori)}")
